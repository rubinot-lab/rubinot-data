...
func parseRubinotDateToUTC(raw string) (string, error) {
	value := strings.TrimSpace(raw)
	if value == "" {
		return "", nil
	}

	for _, layout := range rubinotDateOnlyLayouts {
		parsed, err := time.ParseInLocation(layout, value, rubinotBrazilLocation)
		if err == nil {
			return parsed.UTC().Format(time.RFC3339), nil
		}
	...
	return "", fmt.Errorf("unsupported rubinot date format: %q", raw)
}
